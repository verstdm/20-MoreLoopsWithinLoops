...
def run_test_largest_number():
    ...
    expected = 31
    answer = largest_number(([2, 3, 4], [10, 2, 5], [31, 3, 4]))
    print('Expected and actual are:', expected, answer)
...
def largest_number(seq_seq):
    ...
    for i in range(len(seq_seq)):
        for k in range(len(seq_seq[i])):
            if not str(seq_seq[i][k]) == '':
                c = seq_seq[i][k]
    if str(c) == '':
        return None
    for i in range(len(seq_seq)):
        for k in range(len(seq_seq[i])):
            if int(seq_seq[i][k]) > int(c):
                c = seq_seq[i][k]
    return c
...
def run_test_largest_negative_number():
    ...
    # TO DO 2 (continued): Add your ADDITIONAL test(s) here:
    expected = -5
    answer = largest_negative_number(([2, 3, 4], [-10, 2, -5], [-31, 3, 4]))
    print('Expected and actual are:', expected, answer)
...
def largest_negative_number(seq_seq):
    ...
    for i in range(len(seq_seq)):
        for k in range(len(seq_seq[i])):
            if not str(seq_seq[i][k]) == '' and seq_seq[i][k] < 0:
                c = seq_seq[i][k]
    if str(c) == '':
        return None
    for i in range(len(seq_seq)):
        for k in range(len(seq_seq[i])):
            if int(seq_seq[i][k]) > int(c) and seq_seq[i][k] < 0:
                c = seq_seq[i][k]
    return c
